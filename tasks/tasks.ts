...
dotenv.config();
let {abi} = require("./../artifacts/contracts/MyERC721.sol/MyERC721.json");
...
let contract = new web3.eth.Contract(abi, `${envParams.MY_ERC721}`);
...
async function getSign(obj:SignType, isForStaking?:boolean):Promise<any> {
  //Создаю объект необходимый для подписи транзакций
    return await web3.eth.accounts.signTransaction({
      to:envParams.MY_ERC721,//Адрес контракта, к которому нужно обратиться
